...
# Desc: This program is a basic graphics engine that can display a 3D pyramid on a 2D coordinate plane. There are
        ...
        Lastly, the objects can be shaded based on three different shading models - Lambert, Gouraud, and Phong, which
        also means that the object can be lit with three different lighting models - ambient, emitted, and specular
"""
...
currentObject = [customTube, customPyramid]
...
def resetObject(object):
    ...
    print("resetObject stub executed.")
...
def drawCurrentObject(object):
    ...
    for i in range(len(object.shape)):
        # We also pass in whether or not the object is selected, the position of the polygon in the shape list, and the
        # object itself
...
def createTable(poly, vertexNormals):
    ...
    for i in range(len(edgeList)):
        ...
        if i == 0:
            rowOne = [ymax, ymin, x1, dx, z1, dz, n1, dn, i1, di]
        elif i == 1:
            rowTwo = [ymax, ymin, x1, dx, z1, dz, n1, dn, i1, di]
        else:
            rowThree = [ymax, ymin, x1, dx, z1, dz, n1, dn, i1, di]
...
def reset():
    w.delete(ALL)
    resetObject(currentObject[objectNumber])
    drawAllObjects()
...
# We do the same for the lighting and shading buttons as well
